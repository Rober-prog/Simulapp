...
import { 
    inicializarBD, 
    obtenerAlumnosBD as appObtenerAlumnosBD,
    verificarEstadoBD  
} from './db.js'; 
...
import { inicializarCatalogoFaltas, cargarCatalogoFaltas, repararCatalogoFaltas, verificarEstadoCatalogo } from './catalogoFaltas.js';
...
document.addEventListener('DOMContentLoaded', () => {
    // Verificar estado de almacenamiento local
    const estadoBD = verificarEstadoBD();
    if (estadoBD.status === 'error') {
        mostrarNotificacion('Error al acceder al almacenamiento local. La aplicación podría no funcionar correctamente.', 'error');
        console.error('Error de almacenamiento:', estadoBD.error);
        return;
    }

    // Verificar espacio disponible
    if (estadoBD.storageUsed > estadoBD.storageLimit * 0.9) {
        mostrarNotificacion('Almacenamiento local casi lleno. Considere exportar y limpiar datos antiguos.', 'advertencia');
    }

    // Initialize the database
    inicializarBD();
    
    // Initialize fault catalog with verification
    console.log('Initializing fault catalog...');
    inicializarCatalogoFaltas();
    repararCatalogoFaltas();
    
    const estadoCatalogo = verificarEstadoCatalogo();
    if (estadoCatalogo) {
        console.log(`Catálogo cargado: ${estadoCatalogo.total} faltas totales, ${estadoCatalogo.activas} activas`);
    }
