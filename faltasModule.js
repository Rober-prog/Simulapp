...
function inicializarGestionFaltas() {
    ...
    if (selectAllCheckbox) {
        selectAllCheckbox.addEventListener('change', (e) => {
            actualizarTodasLasFaltas(e.target.checked);
        });
    }
...
// Function to save fault state changes
...
// Updated function to handle "select all" state changes
function actualizarTodasLasFaltas(activar) {
    try {
        const faltas = cargarCatalogoFaltas();
        faltas.forEach(falta => {
            falta.activa = activar;
            const checkbox = document.querySelector(`input[data-id="${falta.id}"]`);
            if (checkbox) {
                checkbox.checked = activar;
                const itemFalta = checkbox.closest('.falta-item-config');
                if (itemFalta) {
                    if (!activar) {
                        itemFalta.classList.add('falta-inactiva');
                    } else {
        ...
        console.error('Error al actualizar todas las faltas:', error);
        mostrarNotificacion('Error al actualizar las faltas', 'error');
    }
}
