...
function mostrarFichaAlumno(alumno) {
    ...
    if (!alumno.simulacros || alumno.simulacros.length === 0) {
        ...
        simulacrosOrdenados.forEach(simulacro => {
            ...
            // Calcular resultado usando la función global
            let resultado = 'NO DETERMINADO';
            if (window.esSimulacroApto) {
                resultado = window.esSimulacroApto(simulacro) ? 'APTO' : 'NO APTO';
            } else {
                // Fallback si esSimulacroApto no está disponible (debería estarlo)
                console.warn("esSimulacroApto no disponible, el resultado del simulacro puede ser incorrecto.");
