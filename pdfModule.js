    ...
    try {
        ...
        if (window.alumnoSeleccionado.simulacros.length === 0) {
            ...
            window.alumnoSeleccionado.simulacros.forEach((simulacro, index) => {
                ...
                let resultado = 'NO DETERMINADO';
                let colorResultado = [0,0,0]; // Negro por defecto
                
                if (window.esSimulacroApto) {
                    if (window.esSimulacroApto(simulacro)) {
                        resultado = 'APTO';
                        colorResultado = [46, 204, 113]; // Verde
                    } else {
                        resultado = 'NO APTO';
                        colorResultado = [231, 76, 60]; // Rojo
                    }
                } else {
                    console.warn("esSimulacroApto no disponible para PDF Ficha Alumno.");
                    // Fallback logic (copiada de la original, debería ser consistente con esSimulacroApto)
                    if (simulacro.faltas.some(f => f.tipo === 'eliminatoria')) {
                         resultado = 'NO APTO'; colorResultado = [231, 76, 60];
                    } else {
                        ...
                        if (deficientes >= 2 || leves >= 10 || (deficientes ===1 && leves >=5) ) {
                            ...
                            resultado = 'APTO'; colorResultado = [46, 204, 113];
                        }
                    }
                }
...
function exportarInformeSimulacro() {
    try {
        ...
        if (window.esSimulacroApto) {
                ...
                if (simulacro.faltas.some(f => f.tipo === 'eliminatoria')) {
                    ...
                    if (deficientes >= 2) {
                        motivoNoApto = '2 o más faltas deficientes';
                         ...
                         motivoNoApto = '1 falta deficiente y 5+ leves';
                    } else if (leves >= 10) {
            ...
            console.warn("esSimulacroApto no disponible para PDF Informe Simulacro.");
            // Fallback logic (copiada, ajustar si esSimulacroApto es más complejo)
            if (simulacro.faltas.some(f => f.tipo === 'eliminatoria')) {
                resultado = 'NO APTO'; colorResultado = [231, 76, 60]; motivoNoApto = 'Falta eliminatoria';
            } else {
                const deficientes = simulacro.faltas.filter(f => f.tipo === 'deficiente').length;
                const leves = simulacro.faltas.filter(f => f.tipo === 'leve').length;
                if (deficientes >= 2) {
                    resultado = 'NO APTO'; colorResultado = [231, 76, 60]; motivoNoApto = '2+ deficientes';
                } else if (deficientes === 1 && leves >= 5) {
                    resultado = 'NO APTO'; colorResultado = [231, 76, 60]; motivoNoApto = '1 deficiente y 5+ leves';
                } else if (leves >= 10) {
                    resultado = 'NO APTO'; colorResultado = [231, 76, 60]; motivoNoApto = '10+ leves';
                } else {
                     resultado = 'APTO'; colorResultado = [46, 204, 113];
                }
            }
