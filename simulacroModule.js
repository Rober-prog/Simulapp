...
function prepararSimulacro() {
    const btnVolver = document.getElementById('btn-volver-simulacro');
    if (!btnVolver) {
        console.error('Button btn-volver-simulacro not found');
        return;
    }
    
    // Remove any existing listeners to prevent duplicates
    const nuevoBoton = btnVolver.cloneNode(true);
    btnVolver.parentNode.replaceChild(nuevoBoton, btnVolver);
    nuevoBoton.addEventListener('click', manejarVolverSimulacro);
    ...
    if (!datosSimulacroAlumno) {
        console.error('Element datos-simulacro-alumno not found');
        return;
    }
    ...
    const tabLeve = document.querySelector('.tab-btn[data-tipo="leve"]');
    if (tabLeve) {
        tabLeve.classList.add('activo');
    }
    
    // Resetear contadores con null checks
    ...
    if (contadorLeves) contadorLeves.textContent = '0';
    if (contadorDeficientes) contadorDeficientes.textContent = '0';
    if (contadorEliminatorias) contadorEliminatorias.textContent = '0';
    
    // Mostrar el botón de iniciar y ocultar el simulacro actual
    const btnIniciar = document.getElementById('btn-iniciar-prueba');
    const btnFinalizar = document.getElementById('btn-finalizar-simulacro');
...
function manejarVolverSimulacro() {
    const simulacroIniciado = window.simulacroActual && tiempoInicio;
    
    if (simulacroIniciado) {
        confirmarAccion('¿Estás seguro de abandonar el simulacro? Los datos no se guardarán.')
            .then(confirmado => {
                if (confirmado) {
                    // Stop simulation and timer
                    detenerCronometro();
        ...
        // If simulation hasn't started, just go back
        if (window.alumnoSeleccionado) {
            window.mostrarFichaAlumno(window.alumnoSeleccionado);
        } else {
            mostrarNotificacion('Error al volver a la ficha del alumno', 'error');
    ...
    if (esApto) {
        ...
        if (totalAptos >= 3) {
            ...
            contenidoResultado.innerHTML = `
                <h2>¡APTO!</h2>
                <p>SIGUE ASÍ...</p>
                <p class="resultado-detalle">Lleva ${totalAptos} de 3 simulacros necesarios para ser candidato a examen</p>
            `;
        }
    } else {
