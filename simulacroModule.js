...
function mostrarInformeSimulacro(simulacro) {
    ...
    if (!alumno) {
        alert('Error: No se pudo encontrar el alumno asociado a este simulacro');
        return;
    }
    ...
    if (simulacro.faltas.some(f => f.tipo === 'eliminatoria')) {
        ...
        if (deficientes >= 2) {
            resultado = 'NO APTO';
            motivoNoApto = '2 faltas deficientes';
        } else if (leves >= 10) {
            resultado = 'NO APTO';
            motivoNoApto = '10 o más faltas leves';
        } else if (leves >= 5 && deficientes >= 1) {
            resultado = 'NO APTO';
            motivoNoApto = '5 o más faltas leves y 1 falta deficiente';
        }
    }
...
function mostrarResultadoFinal() {
    ...
    // Contar simulacros aptos anteriores (incluye el actual si es apto)
    const simulacrosAptos = window.alumnoSeleccionado.simulacros.filter(s => {
        // Check if it's not the current one (to avoid counting it twice)
        if (s.id === simulacro.id && esApto) {
            return false; // Skip current one as we'll add it manually if it's passing
        }
        
        const tieneEliminatoria = s.faltas.some(f => f.tipo === 'eliminatoria');
        const cantDeficientes = s.faltas.filter(f => f.tipo === 'deficiente').length;
        const cantLeves = s.faltas.filter(f => f.tipo === 'leve').length;
        
        return !tieneEliminatoria && cantDeficientes < 2 && cantLeves < 10;
    }).length;
    
    // Add current simulation if it's passing
    const totalAptos = esApto ? simulacrosAptos + 1 : simulacrosAptos;
...
// Make simulacroActual globally available
